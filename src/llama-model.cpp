...
struct llama_model::impl {
    ...
    std::unique_ptr<DStorageSlotManager> dstorage_slots;
};
...
void llama_model_base::load_hparams(llama_model_loader & ml) {
    ...
    if (hparams.n_expert > 0) {
        ...
        if (const char * env = std::getenv("LLAMA_MOE_EXPERT_USED_OVERRIDE")) {
            const int requested = std::atoi(env);
            if (requested > 0 && requested < (int) hparams.n_expert_used) {
                const uint32_t original = hparams.n_expert_used;
                hparams.n_expert_used = (uint32_t) requested;
                LLAMA_LOG_WARN(
                        "%s: experimental MoE expert-used override: %u -> %u active experts per token\n",
                        __func__, original, hparams.n_expert_used);
            } else if (requested != 0) {
                LLAMA_LOG_WARN(
                        "%s: ignoring LLAMA_MOE_EXPERT_USED_OVERRIDE=%d; expected 1..%u\n",
                        __func__, requested, hparams.n_expert_used - 1);
            }
        }
...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    if (params.dstorage_moe) {
        if (arch != LLM_ARCH_QWEN35MOE) {
            throw std::runtime_error("--dstorage-moe currently supports only Qwen MoE models in milestone 1");
        }
        if (devices.empty() || n_gpu_layers <= 0) {
            throw std::runtime_error("--dstorage-moe requires GPU offload");
        }

        pimpl->dstorage_slots = std::make_unique<DStorageSlotManager>();
        if (!pimpl->dstorage_slots->init(
                    hparams.n_layer_all,
                    hparams.n_expert,
                    hparams.n_expert_used,
                    params.moe_gpu_cache_mib,
                    params.moe_pinned_cache_mib,
                    false)) {
            pimpl->dstorage_slots.reset();
            LLAMA_LOG_ERROR("%s: DirectStorage MoE slot manager initialization failed\n", __func__);
            return false;
        }

        ml.dstorage_slots = pimpl->dstorage_slots.get();
        LLAMA_LOG_INFO("%s: DirectStorage MoE slot manager initialized with %u MiB GPU cache and %u MiB pinned cache\n",
                __func__, params.moe_gpu_cache_mib, params.moe_pinned_cache_mib);
    }
...
DStorageSlotManager * llama_model::dstorage_slots() const {
    return pimpl->dstorage_slots.get();
}
...
llama_model_params llama_model_default_params() {
    llama_model_params result = {
        ...
        /*.no_alloc                    =*/ false,
        /*.dstorage_moe                =*/ false,
        /*.moe_gpu_cache_mib           =*/ 1024,
        /*.moe_pinned_cache_mib        =*/ 0,
    };
