...
static std::wstring llama_path_to_wstring(const std::string & path) {
    return std::wstring(path.begin(), path.end());
}

static bool llama_dstorage_parse_expert_tensor(const char * name, int * layer_idx) {
    int parsed_layer = -1;
    char suffix[64] = {};
    if (std::sscanf(name, "blk.%d.%63s", &parsed_layer, suffix) != 2) {
        return false;
    }

    if (std::strcmp(suffix, "ffn_gate_up_exps.weight") != 0 &&
        std::strcmp(suffix, "ffn_down_exps.weight")    != 0 &&
        std::strcmp(suffix, "ffn_gate_exps.weight")    != 0 &&
        std::strcmp(suffix, "ffn_up_exps.weight")      != 0) {
        return false;
    }

    *layer_idx = parsed_layer;
    return true;
}
...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    int dstorage_layer_idx = -1;
    if (dstorage_slots != nullptr &&
            llama_dstorage_parse_expert_tensor(ggml_get_name(cur), &dstorage_layer_idx)) {
        const auto & weight = require_weight(ggml_get_name(cur));
        const std::string & file_path = file_paths.at(weight.idx);
        if (file_path == "(file*)") {
            throw std::runtime_error(format("DirectStorage: expert tensor '%s' is not backed by a filesystem path", ggml_get_name(cur)));
        }

        const int n_experts_real = int(cur->ne[2]);
        if (n_experts_real <= 0 || ggml_nbytes(cur) % size_t(n_experts_real) != 0) {
            throw std::runtime_error(format("DirectStorage: expert tensor '%s' cannot be split by expert", ggml_get_name(cur)));
        }

        ggml_tensor * tensor = ggml_new_tensor_3d(ctx, cur->type, cur->ne[0], cur->ne[1], 1);
        ggml_set_name(tensor, ggml_get_name(cur));
        n_created++;

        LLAMA_LOG_INFO("%s: DirectStorage placeholder tensor '%s' layer=%d experts=%d full=%.2f MiB placeholder=%.2f MiB\n",
                __func__, ggml_get_name(cur), dstorage_layer_idx, n_experts_real,
...
bool llama_model_loader::load_all_data(
        ...
        DStorageSlotManager * dstorage_slots) {
    if (files.empty()) {
    ...
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        ...
        if (dstorage_slots != nullptr &&
                llama_dstorage_parse_expert_tensor(ggml_get_name(cur), &dstorage_layer_idx)) {
            const ggml_tensor * real_tensor = weight->tensor;
            const int real_n_experts = int(real_tensor->ne[2]);
            const size_t real_size = ggml_nbytes(real_tensor);
            if (real_n_experts <= 0 || real_size % size_t(real_n_experts) != 0) {
                throw std::runtime_error(format("DirectStorage: expert tensor '%s' cannot be split by expert", ggml_get_name(cur)));
            }
            const std::string & file_path = file_paths.at(weight->idx);
            if (file_path == "(file*)") {
            ...
            dstorage_slots->register_expert_tensor(
                    dstorage_layer_idx,
                    ggml_get_name(cur),
                    llama_path_to_wstring(file_path),
                    weight->offs,
                    real_size,
                    real_n_experts,
                    0,
                    ggml_row_size(real_tensor->type, real_tensor->ne[0]));

            LLAMA_LOG_INFO("%s: DirectStorage registered expert tensor '%s' layer=%d experts=%d size=%.2f MiB\n",
                    __func__, ggml_get_name(cur), dstorage_layer_idx, real_n_experts,
