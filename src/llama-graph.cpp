...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    if (dstorage_moe) {
        selected_experts_mm = ggml_dup(ctx0, selected_experts);
        cb(selected_experts_mm, "moe_selected_experts_ds", il);
    } else if (moe_routing_trace) {
        selected_experts_mm = ggml_dup(ctx0, selected_experts);
        cb(selected_experts_mm, "moe_selected_experts_trace", il);
    }
...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    if (use_flash_attn) {
        ...
        // CUDA vector flash attention can consume tq3_0 K/V directly for decode.
        // Mixed TQ3/F16 configurations keep the older explicit cast fallback.
        const bool fused_tq3_fattn = k->type == GGML_TYPE_TQ3_0 && v->type == GGML_TYPE_TQ3_0;
        if (k->type == GGML_TYPE_TQ3_0 && !fused_tq3_fattn) {
            k = ggml_cast(ctx0, k, GGML_TYPE_F16);
            cb(k, "k_dequant", il);
        }
        if (v->type == GGML_TYPE_TQ3_0 && !fused_tq3_fattn) {
        ...
        // Dequant tq3_0 K to F32 for the non-flash attention path (mul_mat has no tq3_0 vec_dot)
        ...
        // Dequant quantized V cache (e.g. tq3_0) to F32 for the non-flash attention path
