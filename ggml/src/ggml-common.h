...
// Per TurboQuant (ICLR 2026), PolarQuant stage only (no QJL):
//   per-block WHT32 rotation -> 3-bit Lloyd-Max codebook (8 centroids).
// Each block of 32 values: 3-bit index packed as low 2 bits in qs, high 1 bit in qr,
// plus an fp16 per-block scale (gamma). NB: qr holds the codebook index high bit, NOT QJL signs.
#define QK_TQ3_0 32
typedef struct {
    uint8_t   qs[QK_TQ3_0 / 4]; // low 2 bits of 3-bit index, 32 x 2 bits = 8 bytes
    uint8_t   qr[QK_TQ3_0 / 8]; // high 1 bit of 3-bit index, 32 x 1 bit = 4 bytes
    ggml_half gamma;            // per-block scale (amax / outer centroid)
} block_tq3_0;
