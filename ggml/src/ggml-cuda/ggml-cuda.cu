...
static bool ggml_cuda_should_fuse_mul_mat(const ggml_tensor * ffn_up,
    ...
    static const bool disable_moe_glu_fusion = [] {
        const char * value = std::getenv("LLAMA_CUDA_DISABLE_MOE_GLU_FUSION");
...
static bool ggml_cuda_graph_check_compability(ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (node->name && strstr(node->name, "_ds") != nullptr) {
            use_cuda_graph = false;
#ifndef NDEBUG
            GGML_LOG_DEBUG("%s: disabling CUDA graphs due to DirectStorage MoE node\n", __func__);
#endif
        }
        ...
        if (node->op == GGML_OP_MUL_MAT_ID) {
            ...
            const bool is_dstorage_moe = ids && ids->name && strstr(ids->name, "_ds") != nullptr;
            if (is_dstorage_moe) {
                use_cuda_graph = false;
            } else {
                const int cc = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;
        ...
        case GGML_OP_CPY:
            {
                ...
                if (src0_type == GGML_TYPE_TQ3_0 && src1_type == GGML_TYPE_F16) {
                    return true;
                }
