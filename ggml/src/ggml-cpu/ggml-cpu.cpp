...
static bool ggml_backend_cpu_device_supports_op(ggml_backend_dev_t dev, const struct ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_MUL_MAT:
            {
                // tq3_0 has no CPU vec_dot (KV cache type, consumed on the GPU); don't claim CPU support for it
                const struct ggml_type_traits_cpu * mm_traits = ggml_get_type_traits_cpu(src0->type);
                return mm_traits->vec_dot != NULL &&
                    (src1->type == GGML_TYPE_F32 || src1->type == mm_traits->vec_dot_type);
            }
