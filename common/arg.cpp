...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        [](common_params & params) {
            if (params.dstorage_moe) {
                throw std::invalid_argument("--cpu-moe cannot be combined with --dstorage-moe");
            }
            params.cpu_moe = true;
    ...
    add_opt(common_arg(
        {"-dsm", "--dstorage-moe"},
        "stream Qwen Mixture of Experts (MoE) weights from SSD to GPU with DirectStorage/GDS",
        [](common_params & params) {
            if (params.cpu_moe) {
                throw std::invalid_argument("--dstorage-moe cannot be combined with --cpu-moe");
            }
            params.dstorage_moe = true;
        }
    ).set_examples({LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_PERPLEXITY}).set_env("LLAMA_DSTORAGE_MOE"));
    add_opt(common_arg(
        {"--dstorage-moe-prefetch"},
        "experimentally prefetch future-layer DirectStorage/GDS MoE experts using previous-token routing",
        [](common_params & params) {
            if (params.cpu_moe) {
                throw std::invalid_argument("--dstorage-moe-prefetch cannot be combined with --cpu-moe");
            }
            params.dstorage_moe = true;
            params.dstorage_moe_prefetch = true;
        }
    ).set_examples({LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_PERPLEXITY}).set_env("LLAMA_DSTORAGE_MOE_PREFETCH"));
    add_opt(common_arg(
        {"--moe-gpu-cache-mib"}, "N",
        "MiB of VRAM to use for the DirectStorage MoE expert cache (default: 1024)",
        [](common_params & params, int value) {
            if (value <= 0) {
                throw std::invalid_argument("invalid value");
            }
            params.moe_gpu_cache_mib = uint32_t(value);
        }
    ).set_examples({LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_PERPLEXITY}).set_env("LLAMA_MOE_GPU_CACHE_MIB"));
    add_opt(common_arg(
        {"--moe-pinned-cache-mib"}, "N",
        "MiB of pinned host RAM to use for the DirectStorage/GDS MoE L2 cache (default: 0)",
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("invalid value");
            }
            params.moe_pinned_cache_mib = uint32_t(value);
        }
    ).set_examples({LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_PERPLEXITY}).set_env("LLAMA_MOE_PINNED_CACHE_MIB"));
    add_opt(common_arg(
        {"-ncmoe", "--n-cpu-moe"}, "N",
        ...
        [](common_params & params, int value) {
            ...
            if (value > 0 && params.dstorage_moe) {
                throw std::invalid_argument("--n-cpu-moe cannot be combined with --dstorage-moe");
            }
            if (value > 0) {
                params.cpu_moe = true;
            }
